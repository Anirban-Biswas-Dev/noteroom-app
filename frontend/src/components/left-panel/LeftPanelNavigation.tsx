...
function LeftPanelNavigationItem({ label, icon, extendedClass, onClick }: { label: string, icon: React.ReactElement<SVGElement>, extendedClass?: string, onClick?: any }) {
	return (
		<div className={`item
			w-[90%]
			...
			${extendedClass}
		`} onClick={onClick}>
			<ResizedSVGIconElement icon={icon} />
			<span className="text-[14px] font-['Space_Grotesk'] text-[rgba(0,0,0,0.7)]">{ label }</span>
		</div>
	)
}

function LeftPanelNavigationDropdown({ label, hasIcon=true }: { label: string, hasIcon?: boolean }) {
	return (
		...
		">
			...
			{hasIcon && <ResizedSVGIconElement icon={<icons.downarrow />} />}
		</div>
	)
}

function LeftPanelNavigationButton({ label, icon }: { label: string, icon?: React.ReactElement<SVGElement> }) {
	return (
		<div className="item
			w-[90%]
			flex gap-[15px] items-center
		...
		">
			...
			'>
				{ icon && <ResizedSVGIconElement icon={icon} /> }
				<span className="text-[13px] font-['Space_Grotesk'] text-white font-normal">{ label }</span>
			</button>
		</div>
	)
}
	...
	return (
		<div className="leftpanel-container overflow-y-auto w-[90%] h-full flex flex-col gap-5 items-center">
			...
			<div className="items-container mt-1 xl:mt-0 w-[95%] flex flex-col items-center self-end gap-[1vh]">
				...
				<LeftPanelNavigationItem label='Explore Tools' icon={ <icons.tools /> } />

				<LeftPanelNavigationDropdown label="Others" hasIcon={false} />
				<LeftPanelNavigationItem label='Settings' icon={ <icons.settingsgear /> } />
				<LeftPanelNavigationItem label='Blogs' icon={ <icons.blogs /> } />
			</div>
		</div>
	)
}
